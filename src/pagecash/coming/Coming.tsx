import { useState } from 'react';
import { Button, Table, Modal, Form } from 'react-bootstrap';
import type { Entry } from '../../types';

interface Props {
  data: Entry[];
  setData: React.Dispatch<React.SetStateAction<Entry[]>>;
}

interface FormData {
  item: string;
  amount: string;
  date: string;
  currency: string;
}

const Coming: React.FC<Props> = ({ data, setData }) => {
  const [showModal, setShowModal] = useState(false);
  const [formData, setFormData] = useState<FormData>({
    item: '',
    amount: '',
    date: '',
    currency: 'TJS',
  });

  const handleShow = () => setShowModal(true);
  const handleClose = () => {
    setShowModal(false);
    setFormData({ item: '', amount: '', date: '', currency: 'TJS' });
  };

  const handleChange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>
  ) => {
    setFormData(prev => ({ ...prev, [e.target.name]: e.target.value }));
  };

  const handleSave = () => {
    if (!formData.item.trim() || !formData.amount.trim() || !formData.date.trim()) {
      alert('Заполните все поля!');
      return;
    }

    const amountNumber = Number(formData.amount);
    if (isNaN(amountNumber) || amountNumber <= 0) {
      alert('Введите корректную сумму больше 0');
      return;
    }

    const newEntry: Entry = {
      id: Date.now(),
      item: formData.item,
      amount: amountNumber,
      date: formData.date,
      currency: formData.currency,
    };

    setData(prev => [...prev, newEntry]);
    handleClose();
  };

  return (
    <div>
      <Button variant="success" className="mb-3" onClick={handleShow}>
        Добавить приход
      </Button>

      <Table striped bordered hover responsive className="table-success">
        <thead>
          <tr>
            <th>ID</th>
            <th>Статья дохода</th>
            <th>Сумма</th>
            <th>Валюта</th>
            <th>Дата</th>
          </tr>
        </thead>
        <tbody>
          {data.map(({ id, item, amount, date, currency }) => (
            <tr key={id}>
              <td>{id}</td>
              <td>{item}</td>
              <td>{amount}</td>
              <td>{currency}</td>
              <td>{date}</td>
            </tr>
          ))}
        </tbody>
      </Table>

      <Modal show={showModal} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Добавить приход</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3" controlId="formItem">
              <Form.Label>Статья дохода</Form.Label>
              <Form.Control
                type="text"
                placeholder="Например: Зарплата"
                name="item"
                value={formData.item}
                onChange={handleChange}
              />
            </Form.Group>

            <Form.Group className="mb-3" controlId="formAmount">
              <Form.Label>Сумма</Form.Label>
              <Form.Control
                type="number"
                placeholder="Введите сумму"
                name="amount"
                value={formData.amount}
                onChange={handleChange}
                min={0}
                step="0.01"
              />
            </Form.Group>

            <Form.Group className="mb-3" controlId="formCurrency">
              <Form.Label>Валюта</Form.Label>
              <Form.Select name="currency" value={formData.currency} onChange={handleChange}>
                <option value="TJS">TJS</option>
                <option value="USD">USD</option>
                <option value="RUB">RUB</option>
              </Form.Select>
            </Form.Group>

            <Form.Group className="mb-3" controlId="formDate">
              <Form.Label>Дата</Form.Label>
              <Form.Control type="date" name="date" value={formData.date} onChange={handleChange} />
            </Form.Group>
          </Form>
        </Modal.Body>

        <Modal.Footer>
          <Button variant="secondary" onClick={handleClose}>
            Отмена
          </Button>
          <Button variant="success" onClick={handleSave}>
            Сохранить
          </Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
};

export default Coming;
